...
/* =========================================================================
   GlyphGrid — a line glyph over a short label. The careers pages run on
   these: values, benefits, what a candidate gets. Bare drawings in the
   practice colour on white, no filled container, per the icon rule in the
   design system.
   ========================================================================= */
  ...
  return (
    <Band tone={tone} practice={practice} size="large">
      <SectionHead
        ...
        align={action ? "split" : "left"}
      />
      <ul className={`grid gap-x-12 gap-y-14 ${cols}`}>
        {items.map((item) => (
          <li key={item.title} className="border-t border-line pt-9">
            <span className={`block h-11 w-11 ${glyph}`}>{item.icon}</span>
            <h3 className="mt-8 text-h4 text-ink">{item.title}</h3>
            {item.description && (
              <p className="mt-4 text-sm text-body">{item.description}</p>
            )}
          </li>
        ))}
      </ul>
...
/* =========================================================================
   QuickNav — in-page jump targets. The employee resource centre is four long
   reference sections and an employee arrives wanting exactly one of them, so
   the choice is the first thing on the page rather than a scroll away.
   ========================================================================= */
export function QuickNav({
  items,
  practice = "commercial",
}: {
  items: { label: string; href: string; description?: string; icon: ReactNode }[];
  practice?: Practice;
}) {
  const glyph =
    practice === "government" ? "text-brand-red" : "text-brand-blue";
  ...
  return (
    <section className="relative isolate bg-bg pt-20 sm:pt-24">
      <div className="shell">
        <ul className="grid gap-px border border-line bg-line sm:grid-cols-2 lg:grid-cols-4">
          {items.map((item) => (
            <li key={item.href}>
              ...
              >
                <span className={`block h-9 w-9 ${glyph}`}>{item.icon}</span>
                <span className="mt-7 text-h4 text-ink transition-colors duration-150 ease-brand group-hover:text-link">
                  {item.label}
                </span>
                {item.description && (
                ...
                <span className="mt-auto pt-7 text-muted transition-colors duration-150 ease-brand group-hover:text-link">
                  <Arrow className="h-4 w-4 rotate-90 transition-transform duration-150 ease-brand group-hover:translate-y-1" />
                </span>
              </a>
            </li>
          ))}
...
/* =========================================================================
   ResourceRows — a portal link, what it is for, and the way in. Rows rather
   than cards: the description runs to a full sentence on several of them and
   a card grid would either truncate it or leave dead cells.
   ========================================================================= */
export function ResourceRows({
  ...
  items: {
    title: string;
    description?: string;
    /** Omitted where the destination is an internal URL we do not hold. */
    action?: { label: string; href: string };
    icon?: ReactNode;
  }[];
  tone?: "white" | "tint";
  ...
  return (
    <Band id={id} tone={tone} practice={practice} size="large">
      <SectionHead eyebrow={eyebrow} title={title} lead={lead} practice={practice} />
      ...
      <ul className="border-t border-line">
        {items.map((item) => (
          <li
            key={item.title}
            className="grid items-baseline gap-x-10 gap-y-4 border-b border-line py-8 lg:grid-cols-12"
          >
            <h3 className="flex items-baseline gap-4 text-h4 text-ink lg:col-span-4">
              {item.icon && (
                <span className={`block h-5 w-5 shrink-0 translate-y-1 ${glyph}`}>
                  {item.icon}
                </span>
              )}
              {item.title}
            ...
            <div className="lg:col-span-3 lg:justify-self-end">
              {item.action ? (
                ...
                <span className="text-sm text-muted">Internal portal</span>
              )}
            </div>
          </li>
        ))}
...
/* =========================================================================
   TableBand — a real reference table: holiday dates, pay periods. Hairline
   rows, tabular figures, mono only where the value is a date or a code.
   Scrolls inside its own box on a narrow screen rather than pushing the page
   sideways.
   ========================================================================= */
export function TableBand({
  ...
  columns: string[];
  rows: string[][];
  /** Footnote under the table — the Washington's Birthday note, the 1099 note. */
  note?: ReactNode;
  /**
   * Column indices whose cells are dates or codes. Mono has exactly one job on
   * this site and this is it — the day names beside them stay in the brand sans.
   */
  monoColumns?: number[];
  tone?: "white" | "tint";
  practice?: Practice;
  ...
  return (
    <Band id={id} tone={tone} practice={practice} size="large">
            ...
            <tr className="border-y border-line">
              {columns.map((column) => (
                <th
                  key={column}
                  scope="col"
                  className="py-5 pr-8 text-stat-label uppercase text-muted last:pr-0"
                >
                  {column}
                </th>
              ))}
            ...
            {rows.map((row) => (
              <tr key={row.join("|")} className="border-b border-line">
                {row.map((cell, i) => (
                  <td
                    key={i}
                    className={`py-5 pr-8 align-baseline last:pr-0 ${
                      i === 0 ? "text-base text-ink" : "text-base text-body"
                    } ${
                      monoColumns.includes(i)
                        ? "tabular font-mono text-code"
                        : ""
                    }`}
                  >
                    {cell}
      ...
      {note && <div className="measure mt-8 text-sm text-muted">{note}</div>}
    </Band>
  );
}
